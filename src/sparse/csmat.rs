...
impl<'a, N:'a, I: 'a + SpIndex> CsMat<N, I, Vec<I>, &'a [I], &'a [N]> {
...
impl<'a, N:'a, I: 'a + SpIndex> CsMat<N, I, &'a [I], &'a [I], &'a [N]> {
    ...
    -> Result<CsMatView_<'a, N, I>, SprsError> {
        let m = CsMat {
            storage: storage,
            nrows : shape.0,
            ncols: shape.1,
...
impl<N, I: SpIndex> CsMat<N, I, Vec<I>, Vec<I>, Vec<N>> {
    ...
    fn new_(storage: CompressedStorage,
            ...
            indices : Vec<I>,
            data : Vec<N>
           ) -> Result<CsMatOwned_<N, I>, SprsError>
    where N: Copy
    {
        let mut m = CsMat {
            storage: storage,
    ...
    fn insert_outer_inner(&mut self,
        ...
        if outer_ind >= outer_dims {
            ...
            let location = self.indices[start..stop]
                         .binary_search(&inner_ind_idx);
            match location {
                Ok(ind) => {
...
impl<N: Num> CsMat<N, usize, Vec<usize>, Vec<usize>, Vec<N>> {
...
CsMat<N, I, IptrStorage, IndStorage, DataStorage>
where I: SpIndex,
    ...
    pub fn outer_view(&self, i: usize) -> Option<CsVecView_<N, I>> {
    ...
    /// No allocation required (this is simply a storage order change)
    pub fn transpose_view(&self) -> CsMatView_<N, I> {
        CsMat {
    ...
    pub fn to_owned(&self) -> CsMatOwned_<N, I>
    where N: Clone
    {
        CsMat {
            storage: self.storage,
    ...
    pub fn map<F>(&self, f: F) -> CsMatOwned_<N, I>
    where F: FnMut(&N) -> N,
          N: Clone
    {
    ...
    pub fn view(&self) -> CsMatView_<N, I> {
        CsMat {
            storage: self.storage,
            nrows: self.nrows,
            ncols: self.ncols,
...
where N: Default,
    ...
    pub fn to_other_storage(&self) -> CsMatOwned_<N, I>
    where N: Clone
    {
...
DataStorage: DerefMut<Target=[N]> {
    ...
    where F: FnMut(&mut [I], &mut [I], &mut [N]) {
...
pub mod raw {
    ...
    pub fn convert_storage<N, I>(in_storage: super::CompressedStorage,
                                 shape: Shape,
                                 in_indtpr: &[I],
                                 in_indices: &[I],
                                 in_data: &[N],
    ...
    /// Panics if the output slices don't match the input matrices'
    /// corresponding slices.
    pub fn convert_mat_storage<N: Clone, I: SpIndex>(mat: CsMatView_<N, I>,
                                                     indptr: &mut [I],
                                                     indices: &mut[I],
                                                     data: &mut [N]) {
...
Add<&'b CsMat<N, I, IpS2, IS2, DS2>>
for &'a CsMat<N, I, IpStorage, IStorage, DStorage>
where N: 'a + Copy + Num + Default,
    ...
    type Output = CsMatOwned_<N, I>;

    fn add(self, rhs: &'b CsMat<N, I, IpS2, IS2, DS2>) -> CsMatOwned_<N, I> {
...
where N: 'a + Copy + Num + Default,
    ...
    fn sub(self, rhs: &'b Mat) -> CsMatOwned_<N, I> {
        if self.storage() != rhs.view().storage() {
...
macro_rules! sparse_scalar_mul {
    ($scalar: ident) => (
        ...
        for &'a CsMat<$scalar, I, IpStorage, IStorage, DStorage>
        where I: 'a + SpIndex,
              IpStorage: 'a + Deref<Target=[I]>,
              IStorage: 'a + Deref<Target=[I]>,
              DStorage: 'a + Deref<Target=[$scalar]> {
            type Output = CsMatOwned_<$scalar, I>;

            fn mul(self, rhs: $scalar) -> CsMatOwned_<$scalar, I> {
...
where N: 'a + Copy + Num + Default,
    ...
    fn mul(self, rhs: &'b CsMat<N, I, IpS2, IS2, DS2>) -> CsMatOwned_<N, I> {
...
impl<'a, 'b, N, I, IpS, IS, DS, DS2>
Add<&'b ArrayBase<DS2, Ix2>>
for &'a CsMat<N, I, IpS, IS, DS>
where N: 'a + Copy + Num + Default,
      I: 'a + SpIndex,
...
impl<N, I, IpS, IS, DS> Index<[usize; 2]> for CsMat<N, I, IpS, IS, DS>
where I: SpIndex,
      IpS: Deref<Target=[I]>,
      IS: Deref<Target=[I]>,
      DS: Deref<Target=[N]>
...
impl<N, I, IpS, IS, DS> IndexMut<[usize; 2]> for CsMat<N, I, IpS, IS, DS>
where I: SpIndex,
      IpS: Deref<Target=[I]>,
      IS: Deref<Target=[I]>,
...
impl<N, I, IpS, IS, DS> Index<NnzIndex> for CsMat<N, I, IpS, IS, DS>
where I: SpIndex,
      IpS: Deref<Target=[I]>,
      IS: Deref<Target=[I]>,
      DS: Deref<Target=[N]>
...
impl<N, I, IpS, IS, DS> IndexMut<NnzIndex> for CsMat<N, I, IpS, IS, DS>
where I: SpIndex,
      IpS: Deref<Target=[I]>,
      IS: Deref<Target=[I]>,
      DS: DerefMut<Target=[N]>
